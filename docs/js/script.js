...
function updateSEO() {
  ...
  document
    ...
    .setAttribute("content", seo.keywords.join(", "));
...
function renderNavigation() {
  ...
  sections.forEach((section) => {
    ...
    if (sectionTitle) {
      const existingItem = portfolioData.navigation.find(
...
function renderAboutSection() {
  ...
  document.getElementById("about-extended").textContent =
...
function renderExperienceSection() {
  ...
  sortedYears.forEach((year, yearIndex) => {
    ...
    const sortedExperiencesInYear = experiencesByYear[year].sort(
      (a, b) => new Date(b.starts) - new Date(a.starts),
    );
...
function renderSkillsSection() {
  ...
  skillsGrid.innerHTML = portfolioData.skills.categories
    .map((category) => {
...
function renderReachMeSection() {
  ...
  reachMeContainer.innerHTML = portfolioData.reachMe
    .map(
      (profile, index) => `
  ...
  if (reachMeSection) {
    const reachMeObserver = new IntersectionObserver(
      (entries) => {
...
function renderContactSection() {
  ...
  contactMethods.innerHTML = contact.methods
    .map(
      (method) => `
...
function renderFooter() {
  document.getElementById("footer-copyright").innerHTML =
    `&copy; ${portfolioData.settings.copyright}`;
}
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          for (const [selector, animationClass] of Object.entries(
            animationMappings,
          )) {
          ...
          if (
            element.matches(
              ".project-card, .education-timeline-content, .timeline-content",
            )
          ) {
